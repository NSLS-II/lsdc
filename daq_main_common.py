...
def pybass_init():
    ...
    if sitefilename != "":
        if os.path.exists(sitefilename) == 0:
            error_msg = (
                "\$LSDC_SITE_FILE: %s does not exist. Questionable defaults in effect."
                % sitefilename
            )
    ...
    while 1:
        ...
        if not command:
            ...
            try:
                ...
                error_string = "Unknown command in file: %s Error: %s" % (
                    command_string,
                    e,
                )
...
def run_server():
    _thread.start_new_thread(process_immediate_commands, (0.25,))
    comm_pv = beamline_support.pvCreate(daq_utils.beamlineComm + "command_s")
    beamline_support.pvPut(comm_pv, "\n")
    immediate_comm_pv = beamline_support.pvCreate(
        daq_utils.beamlineComm + "immediate_command_s"
    )
