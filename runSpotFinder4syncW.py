...
if dialsTuneIceRingFlag:
    iceRingParams = " ice_rings.filter=true ice_rings.width=" + str(
        dialsTuneIceRingWidth
    )
else:
    iceRingParams = ""
...
while 1:
    ...
    if localDialsResultDict["data"] == None and retry > 0:
        ...
        if retry == 0:
            ...
            localDialsResultDict["data"]["response"].append(
                {
                    "d_min": "-1.00",
                    "d_min_method_1": "-1.00",
                    "d_min_method_2": "-1.00",
...
for i in range(0, len(localDialsResultDict["data"]["response"])):
    ...
    goodBraggCandidates = localDialsResultDict["data"]["response"][i][
        "spot_count_no_ice"
    ]
    method2Res = localDialsResultDict["data"]["response"][i]["d_min_method_2"]
    totalIntegratedSignal = localDialsResultDict["data"]["response"][i][
        "total_intensity"
    ]
    logfile.write(str(i * 10) + " " + str(goodBraggCandidates) + "\n")
    try:
        ispybLib.insertPlotResult(
            ispybDCID,
            i * 10,
            spotTotal,
            goodBraggCandidates,
